在前面我们学习了Linux操作系统的常见命令以及如何在Linux上部署一个单体项目。大家想一想自己最大的感受是什么？

相信除了个别天赋异禀的同学以外，大多数同学都会有相同的感受，那就是麻烦。核心体现在三点：
...
特别是我们进入微服务阶段学习，微服务项目动辄就是几十台、上百台服务需要部署，有些大型项目甚至达到数万台服务。而**由于每台服务器的运行环境不同，你写好的安装流程、部署脚本并不一定在每个服务器都能正常运行**，经常会出错。这就给系统的部署运维带来了很多困难。
...
![image-20231227114318951](/assets/images/Java/微服务框架/docker/image-20231227114318951.png)
...
![image-20231227141513720](/assets/images/Java/微服务框架/docker/image-20231227141513720.png)
...
![image-20231227153106666](/assets/images/Java/微服务框架/docker/image-20231227153106666.png)
...
![image-20231227155826465](/assets/images/Java/微服务框架/docker/image-20231227155826465.png)
...
>
> 不过，我们发现由于数据卷目录比较深，不好寻找，通常我们也**允许让容器直接与宿主机目录挂载而不使用数据卷**，具体参考2.2.3小节。
...
```JSON
{
  "Mounts": [
    {
      "Type": "volume",
      "Name": "dd1539c52e482dceefde783aadc8565ca79088a5a89b9d4f1729f07162488a03",
      "Source": "/var/lib/docker/volumes/dd1539c52e482dceefde783aadc8565ca79088a5a89b9d4f1729f07162488a03/_data",
...
上述配置是将容器内的`/var/lib/mysql`这个目录，与数据卷`dd1539c52e482dceefde783aadc8565ca79088a5a89b9d4f1729f07162488a03`挂载。于是在宿主机中就有了`/var/lib/docker/volumes/dd1539c52e482dceefde783aadc8565ca79088a5a89b9d4f1729f07162488a03/_data`这个目录。这就是匿名数据卷对应的目录，其使用方式与普通数据卷没有差别。

接下来，可以查看该目录下的MySQL的data文件：

```Bash
ls -l /var/lib/docker/volumes/dd1539c52e482dceefde783aadc8565ca79088a5a89b9d4f1729f07162488a03/_data
```

注意：每一个不同的镜像，将来创建容器后内部有哪些目录可以挂载，可以参考Docke Hub对应的页面。
...
在今天的 `资料` 文件中已经准备好了mysql的`init`目录和`conf`目录：

![image-20231227162502832](/assets/images/Java/微服务框架/docker/image-20231227162502832.png)

上述的文件夹中有对应的初始化SQL脚本和配置文件：

![image-20231227162624793](/assets/images/Java/微服务框架/docker/image-20231227162624793.png)

![image-20231227162703530](/assets/images/Java/微服务框架/docker/image-20231227162703530.png)

其中，`hm.cnf` 主要是配置了MySQL的默认编码，改为utf8mb4；而`hmall.sql`则是后面我们要用到的黑马商城项目的初始化SQL脚本。

我们直接将 `资料/mysql`目录上传至虚拟机的`/root`目录下：

![image-20231227163000364](/assets/images/Java/微服务框架/docker/image-20231227163000364.png)
...
![image-697587b7-34f8-4ff7-b4e3-cc6c97166608](/assets/images/Java/微服务框架/docker/697587b7-34f8-4ff7-b4e3-cc6c97166608.png)
...
![image-20231227170401642](/assets/images/Java/微服务框架/docker/image-20231227170401642.png)

首先，我们将上述的资料提供的`docker-demo.jar`包以及`Dockerfile`拷贝到虚拟机的`/root/demo`目录：

![image-20231227170555806](/assets/images/Java/微服务框架/docker/image-20231227170555806.png)
...
![image-20231227170956134](/assets/images/Java/微服务框架/docker/image-20231227170956134.png)
...
在今天的资料中已经提供了 黑马商城项目给大家，如图：

![image-20231227172503229](/assets/images/Java/微服务框架/docker/image-20231227172503229.png)

项目说明：

- 资料/hmall：商城的后端代码
- 资料/nginx/html/hmall-portal：商城用户端的前端代码
- 资料/nginx/html/hmall-admin：商城管理端的前端代码
...
`hmall`项目（与mybatisPlus课程中的hmall是同一个，可以直接使用）是一个maven聚合项目，使用IDEA打开`hmall`项目，查看项目结构如图：

![image-20231227173118304](/assets/images/Java/微服务框架/docker/image-20231227173118304.png)

我们要部署的就是其中的`hm-service`，其中的配置文件采用了多环境的方式：

![image-20231227173204302](/assets/images/Java/微服务框架/docker/image-20231227173204302.png)

其中的`application-dev.yaml`是部署到开发环境的配置，`application-local.yaml`是本地运行时的配置。

查看application.yaml，你会发现其中的JDBC地址并未写死，而是读取变量：

![image-20231227173302141](/assets/images/Java/微服务框架/docker/image-20231227173302141.png)

这两个变量在`application-dev.yaml`和`application-local.yaml`中并不相同：

![image-20231227173655896](/assets/images/Java/微服务框架/docker/image-20231227173655896.png)

在dev开发环境（也就是Docker部署时）采用了mysql作为地址，刚好是我们的mysql容器名，只要两者在一个网络，就一定能互相访问。

我们将项目打包：

![image-20231227173831707](/assets/images/Java/微服务框架/docker/image-20231227173831707.png)

打包后查看工程结构中，输出的包：

![image-20231227174006169](/assets/images/Java/微服务框架/docker/image-20231227174006169.png)

将`hm-service`目录下的`Dockerfile`和`hm-service/target`目录下的`hm-service.jar`一起上传到虚拟机的`root`目录：

![image-20231227174139394](/assets/images/Java/微服务框架/docker/image-20231227174139394.png)
...
![image-20231227183434386](/assets/images/Java/微服务框架/docker/image-20231227183434386.png)

## 3.2、部署前端

在今天的资料中已经给大家提供了nginx的前端资源、部署目录：

![image-20231227183653002](/assets/images/Java/微服务框架/docker/image-20231227183653002.png)

其中：

- `html`是静态资源目录，包含`hmall-portal`以及`hmall-admin`前端项目
- `nginx.conf`是nginx的配置文件，主要是完成对`html`下的两个静态资源目录做代理

我们现在要做的就是把整个nginx目录上传到虚拟机的`/root`目录下：

![image-20231227183900430](/assets/images/Java/微服务框架/docker/image-20231227183900430.png)
...
![image-20231227184045895](/assets/images/Java/微服务框架/docker/image-20231227184045895.png)
...
```sh
# 1.进入root目录；上传docker-compose.yml到 /root目录
cd /root
