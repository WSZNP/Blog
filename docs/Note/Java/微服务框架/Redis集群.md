...
![image-20240210112635224](/assets/images/Java/微服务框架/Redis集群/image-20240210112635224.png)
...
我们利用`资料\docker-compose.yaml`文件来构建主从集群；文件内容如下：
...
> redis的镜像可以从资料文件夹中加载。 docker load -i redis.tar
>
> network_mode host 容器将不会虚拟出自己的网卡，配置自己的IP等，而是使用宿主机的IP和端口。

将其上传至虚拟机的`/root/redis`目录下：

![image-20240210172850764](/assets/images/Java/微服务框架/Redis集群/image-20240210172850764.png)
...
![image-20240210173040388](/assets/images/Java/微服务框架/Redis集群/image-20240210173040388.png)

查看docker容器，发现都正常启动了：

![image-20240210173144673](/assets/images/Java/微服务框架/Redis集群/image-20240210173144673.png)

由于采用的是host模式，我们看不到端口映射。不过能直接在宿主机通过ps命令查看到Redis进程：

![image-20240210173239946](/assets/images/Java/微服务框架/Redis集群/image-20240210173239946.png)
...
role:master
connected_slaves:2
slave0:ip=192.168.12.168,port=7002,state=online,offset=98,lag=0
slave1:ip=192.168.12.168,port=7003,state=online,offset=98,lag=0
master_failover_state:no-failover
master_replid:62aa0fe8453dea915eb69abfda3a9edbd09f7b44
...
![image-20240210174222437](/assets/images/Java/微服务框架/Redis集群/image-20240210174222437.png)
...
`master`会将自己重新生成一个`replid`标记为新的主节点`replid`；然后和`offset`都发送给这个`slave`，`slave`保存这些信息到本地。自此以后`slave`的`replid`就与`master`一致了。

因此，**master判断一个节点是否是第一次同步的依据，就是看replid是否一致**。流程如图：

![image-20240210174317518](/assets/images/Java/微服务框架/Redis集群/image-20240210174317518.png)
...
运行 `docker logs -f r1` 来看下`r1`节点的运行日志：

![image-20240210203529567](/assets/images/Java/微服务框架/Redis集群/image-20240210203529567.png)

运行 `docker logs -f r2` 再看下`r2`节点执行`replicaof`命令时的日志：

![image-20240210204304306](/assets/images/Java/微服务框架/Redis集群/image-20240210204304306.png)
...
![image-20240210204351993](/assets/images/Java/微服务框架/Redis集群/image-20240210204351993.png)
...
这就要说到增量同步时的`repl_baklog`文件了。这个文件是一个固定大小的数组，只不过数组是环形，也就是说**角标到达数组末尾后，会再次从0开始读写**，这样数组头部的数据就会被覆盖。

`repl_baklog`中会记录Redis处理过的命令及`offset`，包括master当前的`offset`，和slave已经拷贝到的`offset`：

![image-20240210204440836](/assets/images/Java/微服务框架/Redis集群/image-20240210204440836.png)

slave与master的offset之间的差异，就是salve需要增量拷贝的数据了。

随着不断有数据写入，master的offset逐渐变大，slave也不断的拷贝，追赶master的offset：

![image-20240210204519584](/assets/images/Java/微服务框架/Redis集群/image-20240210204519584.png)

直到数组被填满：

![image-20240210204543564](/assets/images/Java/微服务框架/Redis集群/image-20240210204543564.png)

此时，如果有新的数据写入，就会覆盖数组中的旧数据。不过，旧的数据只要是绿色的，说明是已经被同步到slave的数据，即便被覆盖了也没什么影响。因为未同步的仅仅是红色部分：

![image-20240210204611224](/assets/images/Java/微服务框架/Redis集群/image-20240210204611224.png)

但是，如果slave出现网络阻塞，导致master的`offset`远远超过了slave的`offset`：

![image-20240210204635781](/assets/images/Java/微服务框架/Redis集群/image-20240210204635781.png)

如果master继续写入新数据，master的`offset`就会覆盖`repl_baklog`中旧的数据，直到将slave现在的`offset`也覆盖：

![image-20240210204714651](/assets/images/Java/微服务框架/Redis集群/image-20240210204714651.png)
...
![image-20240210205255863](/assets/images/Java/微服务框架/Redis集群/image-20240210205255863.png)
...
![image-20240210210129753](/assets/images/Java/微服务框架/Redis集群/image-20240210210129753.png)
...
![image-20240210210414886](/assets/images/Java/微服务框架/Redis集群/image-20240210210414886.png)
...
答案是**第一个确认master**客观下线的人会立刻发起**投票**，一定会成为**leader**。
...
![image-20240210211521700](/assets/images/Java/微服务框架/Redis集群/image-20240210211521700.png)

假如master发生故障，slave1当选。则故障转移的流程如下：

1）`sentinel`给备选的`slave1`节点发送`slaveof no one`命令，让该节点成为`master`

![image-20240210211622193](/assets/images/Java/微服务框架/Redis集群/image-20240210211622193.png)

2）`sentinel`给所有其它`slave`发送`slaveof 192.168.12.168 7002` 命令，让这些节点成为新`master`，也就是`7002`的`slave`节点，开始从新的`master`上同步数据。

![image-20240210212026799](/assets/images/Java/微服务框架/Redis集群/image-20240210212026799.png)

3）最后，当故障节点恢复后会接收到哨兵信号，执行`slaveof 192.168.12.168 7002`命令，成为`slave`：

![image-20240210212053016](/assets/images/Java/微服务框架/Redis集群/image-20240210212053016.png)
...
然后，我们找到`资料\sentinel.conf`文件：

其内容如下：

```Bash
sentinel announce-ip "192.168.12.168"
sentinel monitor hmaster 192.168.12.168 7001 2
...
- `sentinel announce-ip "192.168.12.168"`：声明当前sentinel的ip
- `sentinel monitor hmaster 192.168.12.168 7001 2`：指定集群的主节点信息
  - `hmaster`：主节点名称，自定义，任意写
  - `192.168.12.168 7001`：主节点的ip和端口
  - `2`：认定`master`下线时的`quorum`值
- `sentinel down-after-milliseconds hmaster 5000`：声明master节点超时多久后被标记下线
- `sentinel failover-timeout hmaster 60000`：故障转移的超时时间

我们在虚拟机的`/root/redis`目录下新建3个文件夹：`s1`、`s2`、`s3`:

![image-20240210212727804](/assets/images/Java/微服务框架/Redis集群/image-20240210212727804.png)

将`资料\sentinel.conf`文件分别拷贝一份到上述3个文件夹中。
...
services:
  ...
  r2:
    ...
    container_name: r2
    network_mode: "host"
    entrypoint: ["redis-server", "--port", "7002", "--slaveof", "192.168.12.168", "7001"]
  r3:
    image: redis
    container_name: r3
    network_mode: "host"
    entrypoint: ["redis-server", "--port", "7003", "--slaveof", "192.168.12.168", "7001"]
  s1:
    image: redis
    container_name: s1
    volumes:
...
![image-20240210213307390](/assets/images/Java/微服务框架/Redis集群/image-20240210213307390.png)
...
Sentinel ID is 2a75518cb93b86e966605c087824501f58c39450
+monitor master hmaster 192.168.12.168 7001 quorum 2
+sentinel sentinel b930a3f459a001b569d2bbcdc4347d339b028d29 192.168.12.168 27002 @ hmaster 192.168.12.168 7001
+sentinel sentinel f26bcb9eda6bf8e09baa9259ab4a80b50dd3afa6 192.168.12.168 27003 @ hmaster 192.168.12.168 7001
+slave slave 192.168.12.168:7002 192.168.12.168 7002 @ hmaster 192.168.12.168 7001
+slave slave 192.168.12.168:7003 192.168.12.168 7003 @ hmaster 192.168.12.168 7001
```

可以看到`sentinel`已经联系到了`7001`这个节点，并且与其它几个哨兵也建立了链接。哨兵信息如下：

- `27001`：`Sentinel ID`是`2a75518cb93b86e966605c087824501f58c39450`
- `27002`：`Sentinel ID`是`b930a3f459a001b569d2bbcdc4347d339b028d29`
- `27003`：`Sentinel ID`是`f26bcb9eda6bf8e09baa9259ab4a80b50dd3afa6`

## 2.3、演示failover

接下来，我们演示一下当主节点故障时，哨兵是如何完成集群故障恢复（failover）的。

我们停止`7001`这个`master`节点，然后再启动模拟宕机重启：

```Bash
docker stop r1
```

稍微等待一段时间后，会发现sentinel节点触发了`failover`：可以使用 `docker logs -f s1` 等查看各个sentinel容器的日志：

![image-20240210223142729](/assets/images/Java/微服务框架/Redis集群/image-20240210223142729.png)

![image-20240210223314780](/assets/images/Java/微服务框架/Redis集群/image-20240210223314780.png)
...
![image-20240210235428784](/assets/images/Java/微服务框架/Redis集群/image-20240210235428784.png)
...
![image-20240210235542320](/assets/images/Java/微服务框架/Redis集群/image-20240210235542320.png)
...
| ------ | ------ | -------------- | -------- |
| r1     | master | 192.168.12.168 | 7001     |
| r2     | master | 192.168.12.168 | 7002     |
| r3     | master | 192.168.12.168 | 7003     |
| r4     | slave  | 192.168.12.168 | 7004     |
| r5     | slave  | 192.168.12.168 | 7005     |
| r6     | slave  | 192.168.12.168 | 7006     |
...
ps -ef | grep redis
# 结果：
[root@itheima redis-cluster]# ps -ef | grep redis
root     126860 126809  0 01:35 ?        00:00:00 redis-server *:7002 [cluster]
root     126924 126840  0 01:35 ?        00:00:00 redis-server *:7003 [cluster]
root     126949 126836  0 01:35 ?        00:00:00 redis-server *:7004 [cluster]
root     126981 126891  0 01:35 ?        00:00:00 redis-server *:7006 [cluster]
root     126989 126910  0 01:35 ?        00:00:00 redis-server *:7001 [cluster]
root     126990 126918  0 01:35 ?        00:00:00 redis-server *:7005 [cluster]
...
![image-20240211013936495](/assets/images/Java/微服务框架/Redis集群/image-20240211013936495.png)

这里展示了集群中`master`与`slave`节点分配情况，并询问你是否同意。节点信息如下：

- `7001`是`master`，节点`id`后6位是`34dbb8`
- `7002`是`master`，节点`id`后6位是`22d8ae`
- `7003`是`master`，节点`id`后6位是`51f9c3`
- `7004`是`slave`，节点`id`后6位是`2713d6`，认`51f9c3`（7003）为`master`
- `7005`是`slave`，节点`id`后6位是`3c6899`，认`34dbb8`（7001）为`master`
- `7006`是`slave`，节点`id`后6位是`aba4ad`，认`22d8ae`（7002）为`master`

输入`yes`然后回车。会发现集群开始创建，并输出下列信息：

![image-20240211014245648](/assets/images/Java/微服务框架/Redis集群/image-20240211014245648.png)
...
![image-20240211014355665](/assets/images/Java/微服务框架/Redis集群/image-20240211014355665.png)
...
![image-20240211014516776](/assets/images/Java/微服务框架/Redis集群/image-20240211014516776.png)
...
![image-20240211014828184](/assets/images/Java/微服务框架/Redis集群/image-20240211014828184.png)
...
![image-20240211014923128](/assets/images/Java/微服务框架/Redis集群/image-20240211014923128.png)
...
![image-20240211015340798](/assets/images/Java/微服务框架/Redis集群/image-20240211015340798.png)
...
![image-20240211015637223](/assets/images/Java/微服务框架/Redis集群/image-20240211015637223.png)

命令前面的watch可以每隔一段时间刷新执行结果，方便我们实时监控集群状态变化。

接着，我们故技重施，利用命令让某个master节点停止。比如这里我们让`7002`节点停止，打开一个新的ssh控制台，输入下面命令：
...
![image-20240211015832620](/assets/images/Java/微服务框架/Redis集群/image-20240211015832620.png)

过了一段时间后，7002原本的小弟7006变成了`master`：

![image-20240211015901958](/assets/images/Java/微服务框架/Redis集群/image-20240211015901958.png)

再次启动 `docker start r2` 而7002被标记为`slave`，而且其`master`正好是7006，主从地位互换。

![image-20240211015952115](/assets/images/Java/微服务框架/Redis集群/image-20240211015952115.png)
...
![image-20240211105355037](/assets/images/Java/微服务框架/Redis集群/image-20240211105355037.png)
...
![image-20240211110111425](/assets/images/Java/微服务框架/Redis集群/image-20240211110111425.png)

我们可以看到1号元素就有指向3、5、10的多个指针，查询时就可以跳跃查找。例如我们要找大小为14的元素，查找的流程是这样的：

![image-20240211110139557](/assets/images/Java/微服务框架/Redis集群/image-20240211110139557.png)
...
每个节点中都包含ele和score两个属性，其中score是得分，也就是节点排序的依据。ele则是节点存储的数据。

其内存结构如下：

![image-20240211110229754](/assets/images/Java/微服务框架/Redis集群/image-20240211110229754.png)
...
```C
typedef struct zset {
    dict *dict; // dict，底层就是HashTable。。根据元素查询时使用
    zskiplist *zsl; // 跳表；根据分数查排序、位置时，利用跳表特性快速定位
} zset;
```

其内存结构如图：

![image-20240211110318267](/assets/images/Java/微服务框架/Redis集群/image-20240211110318267.png)
...
Redis不管有多少种数据类型，本质是一个`KEY-VALUE`的键值型数据库，而这种键值映射底层正是基于HashTable来实现的，在Redis中叫做Dict.
...
**周期删除**：顾明思议是通过一个定时任务，周期性的**抽样**部分过期的key，然后执行删除。

执行周期有两种：

- **SLOW模式：**Redis会设置一个定时任务`serverCron()`，按照`server.hz`的频率来执行过期key清理
- **FAST模式：**Redis的每个事件循环前执行过期key清理（事件循环就是NIO事件处理的循环，事件循环的事件比如客户端连接请求、命令执行、定时任务等）。
...
![image-20240211172159830](/assets/images/Java/微服务框架/Redis集群/image-20240211172159830.png)
...
![image-20240211172249279](/assets/images/Java/微服务框架/Redis集群/image-20240211172249279.png)
...
![image-20240211201425983](/assets/images/Java/微服务框架/Redis集群/image-20240211201425983.png)

**异常情况**

![image-20240211201503612](/assets/images/Java/微服务框架/Redis集群/image-20240211201503612.png)
...
![image-20240211201556533](/assets/images/Java/微服务框架/Redis集群/image-20240211201556533.png)

**异常情况**

![image-20240211201833736](/assets/images/Java/微服务框架/Redis集群/image-20240211201833736.png)
...
**综上**，添加缓存的目的是为了提高系统性能，而你要付出的代价就是缓存与数据库的强一致性。如果你要求数据库与缓存的强一致，那就需要**加锁**避免并行读写。但这就降低了性能，与缓存的目标背道而驰。

因此不管任何缓存同步方案最终的目的都是尽可能保证最终一致性，降低发生不一致的概率。我们**采用先更新数据库再删除缓存**的方案，已经将这种概率降到足够低，目的已经达到了。
...
假如有不怀好意的人，开启很多线程频繁的**访问一个数据库中也不存在的数据。由于缓存不可能生效，那么所有的请求都访问数据库，可能就会导致数据库因过高的压力而宕机**。
...
简单来说，就是当我们发现请求的数据即不存在于缓存，也不存在于数据库时，将空值缓存到Redis，避免频繁查询数据库。实现思路如下：

![image-20240211202221669](/assets/images/Java/微服务框架/Redis集群/image-20240211202221669.png)
...
![image-20240211202550170](/assets/images/Java/微服务框架/Redis集群/image-20240211202550170.png)
...
![image-20240211202619856](/assets/images/Java/微服务框架/Redis集群/image-20240211202619856.png)
...
![image-20240211204752822](/assets/images/Java/微服务框架/Redis集群/image-20240211204752822.png)

## 6.3、缓存雪崩

缓存雪崩是指在**同一时段大量的缓存key同时失效或者Redis服务宕机，导致大量请求到达数据库，带来巨大压力**。

![image-20240211205844151](/assets/images/Java/微服务框架/Redis集群/image-20240211205844151.png)
...
![image-20240211205934647](/assets/images/Java/微服务框架/Redis集群/image-20240211205934647.png)
...
![image-20240211210015290](/assets/images/Java/微服务框架/Redis集群/image-20240211210015290.png)

逻辑过期的思路如图：

![image-20240211210042276](/assets/images/Java/微服务框架/Redis集群/image-20240211210042276.png)

## 6.5、面试总结

**面试题**：**如何保证缓存的双写一致性**？

**答**：缓存的双写一致性很难保证强一致，只能尽可能降低不一致的概率，确保最终一致。我们项目中采用的是`Cache Aside`模式。简单来说，就是在**更新数据库之后删除缓存**；在查询时先查询缓存，如果未命中则查询数据库并写入缓存。同时我们会给缓存设置过期时间作为兜底方案，如果真的出现了不一致的情况，也可以通过缓存过期来保证最终一致。
...
>
>    https://mp.weixin.qq.com/s/JGikS9Y7PrelYuHLh3R2Ow

**面试题**：**如何解决缓存穿透问题**？

**答**：缓存穿透也可以说是穿透攻击，具体来说是因为请求**访问到了数据库不存在的值**，这样缓存无法命中，必然访问数据库。如果高并发的访问这样的接口，会给数据库带来巨大压力。
...
**答**：缓存击穿往往是由热点Key引起的，当**热点Key过期**时，大量请求涌入同时查询，发现缓存未命中都会去访问数据库，导致数据库压力激增。解决这个问题的主要思路就是避免多线程并发去重建缓存，因此方案有两种。
